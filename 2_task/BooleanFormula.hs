...
linearForm1 = C Xor [V 0, V 1]
nonLinearForm1 = C Xor [C Xor [V 0, V 1], C And [V 0, V 1]]
...
getNextLine :: [Bool] -> [Bool]
getNextLine [x] = [x]
getNextLine (x:xs) = zipWith xor xs $ withoutLast (x:xs)
    where withoutLast = reverse . drop 1 . reverse

pascal :: [Bool] -> [[Bool]]
pascal [x] = [[x]]
pascal l = (l : (pascal $ getNextLine l))

linear :: Formula -> Bool
linear f = not $ any (True==) $ zipWith (&&) nonLinearList jegalkinCoefficients
    where jegalkinCoefficients = map (head) (pascal $ formulaValues f)
          nonLinearList = [(length $ filter (==True) env) > 1 | env <- allFormulaEnvs f]
...
rf d n =
--    do prob <- randomRange (1, 6)
    do op <- frequency [(1, return Neg), (5, randomBin)]
       if op == Neg
       then do f <- rf (d-1) n
               return (C Neg [f])
       else do f1 <- rf (d-1) n
